...
struct SelfieView: BaseView {
                        ...
                        if count > 1.5 || !showCircleProgress {
                            ...
                            if let image = delegate?.lastImage {
                                ...
                                if PSDKSession.shared.withFlow() {
                                    store.dispatch(ModulePreventorAction.biometrics(.nextScreen()))
                                } else {
                                    store.dispatch(ModulePreventorAction.biometrics(.showValidatingView))
                                }
                            }
                        }
                    }
    ...
    func updateNavigationSettings() {
        if let coordinator = store.coordinator as? ModulePreventorCoordinator,
