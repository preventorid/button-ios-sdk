...
struct PersonalInfoView: BaseView {
    ...
    init(store: ReduxStore<PersonalInfoState>) {
        self.store = store
        self.canEdit = PSDKSession.shared.isStreamline
    ...
    func nextAction() {
        if canEdit {
            if PSDKSession.shared.withFlow() {
                store.parent?.dispatch(ModulePreventorAction.streamline(.nextScreen))
            } else {
                store.parent?.dispatch(ModulePreventorAction.streamline(.showStepEmail))
            }
        } else {
            store.parent?.dispatch(ModulePreventorAction.showFinishApp(
                screen: getFinishAppScreen()))
        }
    }
    
    func getFinishAppScreen() ->FinishAppState.Screen {
        PSDKSession.shared.biometricsResult == .SUCCESSFUL ? .verifiedBiometrics : .awaitingBiometrics
    }
    
    func prevAction() {
        store.parent?.dispatch(ModulePreventorAction.streamline(.showEditInfo))
    }
