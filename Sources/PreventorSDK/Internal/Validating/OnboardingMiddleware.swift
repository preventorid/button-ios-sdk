...
final class OnboardingMiddleware: PSDKReduxMiddleware<PSDKEmptyState> {
        ...
        repository.onboarding(
            ...
            success: { [weak self] result in
                if result.status == 400 {
                    self?.store?.parent?.dispatch(ModulePreventorAction.biometrics(
                        .showBiometricsResult(resultType: .errorScanDocument)))
                    return
                }
    ...
    private func onCachedFailed(response: UserDataResponse) {
        ...
        case .GET_TOKEN, .PHOTO_VERIFICATION, .PORTRAIT_UPLOAD, .ENROLLMENT:
            self.store?.parent?.dispatch(ModulePreventorAction.biometrics(
                .showBiometricsResult(resultType: .errorSelfie)))
        }
    }
        ...
        self.repository.cachedTransaction(
            ...
            success: { [weak self] response in
                ...
                case .AWAITING, .VERIFIED:
                    ...
                    PSDKSession.shared.biometricsResult = transactionStatus
                    self?.store?.parent?.dispatch(ModulePreventorAction.streamline(.showPersonalInfo))
                    if PSDKSession.shared.isStreamline {
