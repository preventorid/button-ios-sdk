    ...
    func nextAction() {
        if let documentType = item?.type {
            ...
            if PSDKSession.shared.withFlow() {
                store.parent?.dispatch(ModulePreventorAction.biometrics(.nextScreen(true)))
            } else {
                store.parent?.dispatch(ModulePreventorAction.biometrics(.showReadyScan(type: documentType)))
            }
        }
    }
